// Exemplo contador. Sem state ou map

import "./App.css";
import { useState } from "react";

function AppV1() {
  const [variavelObservavel, funcaoQueAtualizaOEstado] = useState(0);

  function AdicionarMaisUmaUnidade() {
    // variavelObservavel += 1; // Não permitido mudar o valor diretamente. O React não "vê" essa mudança e por isso é necessário usar a função que atualiza o estado.
    funcaoQueAtualizaOEstado(variavelObservavel + 1);
  }
  ...
  return (
    <div className="min-h-screen w-screen bg-[#05080f] space-y-11 flex flex-col items-center">
      ...
      {/* EXEMPLO CONTADOR */}
      <div className="text-slate-200 bg-[#0F1421] p-8 w-2/3">
        <p>Você clicou {variavelObservavel} vezes</p>
        <button
          className="bg-gradient-to-r from-[#1E3A8A] to-[#2563EB] text-black py-3 rounded-lg font-medium flex gap-2 w-full justify-center"
          onClick={() => AdicionarMaisUmaUnidade()}
        >
